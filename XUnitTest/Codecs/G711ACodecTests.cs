...
public class G711ACodecTests
{
    private readonly G711ACodec _codec = new();

    [Fact(DisplayName = "G711A编码→解码往返，验证PCM数据一致")]
    ...
    [Fact(DisplayName = "G711A编码已知测试向量：0→0xD5")]
    public void Encode_Zero_ReturnsD5()
    {
        var pcm = new Byte[] { 0, 0 }; // PCM 0
        var result = _codec.FromPcm(pcm, null);
        Assert.Equal(0xD5, result[0]);
    }

    [Fact(DisplayName = "G711A解码已知测试向量：0xD5→PCM0")]
    public void Decode_D5_ReturnsZero()
    {
        var encoded = new Byte[] { 0xD5 };
    ...
    [Fact(DisplayName = "G711A编码静音缓冲区输出均为0xD5")]
    public void Encode_Silence_AllD5()
    {
        var pcm = new Byte[200];
        var result = _codec.FromPcm(pcm, null);
        for (var i = 0; i < result.Total; i++)
            Assert.Equal(0xD5, result[i]);
    }

    [Fact(DisplayName = "G711A编码所有16位值不抛异常")]
